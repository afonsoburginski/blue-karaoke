...
export const users = pgTable("users", {
  ...
  slug: text("slug").unique(), // Opcional - será gerado no hook afterCreate
  ...
  // password não está aqui - Better Auth armazena na tabela 'account'
...
export const musicas = pgTable("musicas", {
  ...
  userId: text("user_id").references(() => users.id).notNull(), // usuário que fez upload
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
})
...
export const sincronizacoes = pgTable("sincronizacoes", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: text("user_id").references(() => users.id).notNull(), // Usuário/máquina que sincronizou
